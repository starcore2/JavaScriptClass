...
function subtractNumbers()
{
    ...
    let dif = function(minuend,subtrahend){return minuend-subtrahend};
    document.querySelector("#difference").value=dif(minuend,subtrahend);
}
...
let odd=[];
let even=[];
odd = x.filter(check=> check % 2 == 1);
even = x.filter(check=> check % 2 == 0);
...
document.querySelector("#sumOfArray").textContent=total;
double =x.map(content=>content*2)
document.querySelector("#multiplied").textContent = double;
document.querySelector("#sumOfMultiplied").textContent = double.reduce(reducer);